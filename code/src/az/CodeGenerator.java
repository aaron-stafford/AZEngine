...
public class CodeGenerator extends AZGenericMachineGenerator
{
	...
	private static void processArgs(String[] args)
	{
		...
		{
			System.err
			        .println("When specifying a template to use, excatly one file type must also be specified");
			System.exit(1);
		}
		...
		{
			System.err
			        .println("Specify a file type with --file-type to use an in build template. Otherwise supply a custom template with --template");
			System.exit(1);
		}
		...
		{
			System.err
			        .println("Warning: --not-derived has no effect when using a custom template");
		}
		
		if (STATE_DIAGRAM == null)
		{
			System.err
			        .println("No diagram specified. Sepcify one with --diagram");
			System.exit(1);
		}
